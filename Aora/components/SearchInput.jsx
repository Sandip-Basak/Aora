...
const SearchInput = ({initialQuery}) => {
  ...
  return (
        ...
        flex-row space-x-4 mt-3'>
            <TextInput 
            className='text-base mt-0.5 text-white flex-1 font-pregular'
            value={query} placeholder="Search for a video topic" placeholderTextColor="#cdcde0"
            onChangeText={(e)=>setQuery(e)}/>
            <TouchableOpacity
                onPress={()=>{
                    if(!query){
                        ToastAndroid.show('Search field empty', ToastAndroid.SHORT)
                    }
                    else{
